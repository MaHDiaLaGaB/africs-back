...
def compute_amount_lyd(
    amount_foreign: float, service_price: float, operation: str
) -> float:
...
def create_transaction(
    db: Session, data: TransactionCreate, employee: User
) -> Transaction:
    service = (
        db.query(Service)
    ...
    if service.operation in ("multiply", "pluse"):
        ...
        raise HTTPException(
            status_code=400, detail=f"Unsupported operation {service.operation}"
        )
    ...
    amount_lyd_expected = compute_amount_lyd(
        data.amount_foreign, service.price, service.operation
    )
...
def update_transaction_status(
    ...
    if not txn:
        logger.warning(
            "[%03d]     Transaction %s not found - aborting", call_id, transaction_id
        )
    ...
    if (
        ...
        and old_status != TransactionStatus.cancelled
    ):
        original_amount_lyd = txn.amount_lyd
        logger.info(
            "[%03d]     Cancellation branch (original_amount_lyd=%s)",
            call_id,
            original_amount_lyd,
        )
        ...
        if txn.payment_type == PaymentType.cash:
            ...
            logger.info(
                "[%03d]     Adjusting customer #%s balance_due by -%s",
                call_id,
                txn.customer_id,
                original_amount_lyd,
            )
            ...
            if customer:
                ...
                logger.warning(
                    "[%03d]     Customer %s not found - skipped balance adjustment",
                    call_id,
                    txn.customer_id,
                )
    ...
    logger.info(
        "[%03d] ◀ Exit update_transaction_status (returned txn #%s)", call_id, txn.id
    )
...
def update_transaction(
    db: Session, txn_id: int, data: TransactionUpdate, modified_by: int
) -> Transaction:
    ...
    if data.amount_foreign is not None and data.amount_foreign != txn.amount_foreign:
        ...
        if delta_foreign > 0:
            ...
            if txn.payment_type == PaymentType.cash:
                ...
                customer = (
                    db.query(Customer).filter(Customer.id == txn.customer_id).first()
                )
                if customer:
    ...
    if status is not None and status != txn.status:
        txn = update_transaction_status(db, txn.id, status, reason, modified_by)
