...
def upgrade() -> None:
    ...
    sa.Index(op.f("ix_users_username"), users.c.username, unique=True).create(
    ...
    sa.Index(op.f("ix_receipt_orders_id"), receipt_orders.c.id).create(
        bind, checkfirst=True
    )
    ...
    services = sa.Table(
        ...
        sa.Column(
            "operation",
            sa.Enum("multiply", "divide", "pluse", name="operationtype"),
            nullable=False,
        ),
    ...
    sa.Index(op.f("ix_treasury_transfers_id"), treasury_transfers.c.id).create(
        bind, checkfirst=True
    )
    ...
    transactions = sa.Table(
        ...
        sa.Column(
            "payment_type", sa.Enum("cash", "credit", name="paymenttype"), nullable=True
        ),
        sa.Column(
            "status",
            sa.Enum("pending", "completed", "cancelled", name="transactionstatus"),
            nullable=True,
        ),
    ...
    sa.Index(op.f("ix_transactions_id"), transactions.c.id).create(
        bind, checkfirst=True
    )
    sa.Index(
        op.f("ix_transactions_reference"), transactions.c.reference, unique=True
